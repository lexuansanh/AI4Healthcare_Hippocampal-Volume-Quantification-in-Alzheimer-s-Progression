...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: batch size, predicted classes, coronal, axial
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                # Convert image data to float and move it to the device (GPU/CPU)
                data = batch['image'].float().to(self.device)

                # Move segmentation target to the device (GPU/CPU)
                target = batch['seg'].to(self.device)

                # Make predictions using the model
                prediction = self.model(data)

                # Apply softmax activation to the predictions along dimension 1
                prediction_softmax = F.softmax(prediction, dim=1)

                # Compute the loss using the predictions and the target, considering only the first channel
                loss = self.loss_function(prediction, target[:, 0, :, :])
